...
class cnn_lstm_cat(nn.Module):
    def __init__(self):
        super(cnn_lstm_cat, self).__init__()
        self.cnn = CNN1DEncoder()
        self.rnn = nn.LSTM(
            input_size=80, 
            hidden_size=80, 
        ...
        # self.static_encoder = nn.Linear(160+129, 512)
        self.node_number = 0
    def forward(self, x,static):
        x = self.cnn(x)
        self.node_number = self.cnn.node_number
        # s_feature = self.static_encoder(static).view(self.node_number, 128).contiguous()
        # static = static.view(self.node_number, 129).contiguous()
...
class cnn_lstm_attention_gat(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(160,2)
        self.dy_enc = nn.Linear(160, 256)
        self.st_enc = nn.Linear(129, 256)
    ...
    def forward(self, xss,static):
        ...
        feat = torch.sum(scored_x, dim=1) #加权求和
        feat = self.dy_enc(feat)
        st = self.st_enc(static.view(self.cnn_lstm.node_number, 129).contiguous())
        outs = torch.cat([feat, st], dim=-1)
        
        adj = self.makeadj(outs)
        
        outs = outs.view(self.cnn_lstm.node_number,512)
        adj = adj.view(self.cnn_lstm.node_number,self.cnn_lstm.node_number)
        

        gout, _, _ = self.gnn(outs,adj)
        out = gout
        # gout = gout.view(-1,160)
